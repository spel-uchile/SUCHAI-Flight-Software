...
#endif
{
    ...
    executer_stat_queue = osQueueCreate(2,sizeof(int));
    if(executer_stat_queue == 0)
        LOGE(tag, "Error creating executer stat queue");
    executer_cmd_queue = osQueueCreate(2,sizeof(cmd_t *));
    ...
    osCreateTask(taskExecuterA, "executer_A", 5*256, NULL, 4, &threads_id[2]);
